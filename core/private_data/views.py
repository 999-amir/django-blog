...
class PrivateDataView(View):
    ...
    def dispatch(self, request, *args, **kwargs):
        if not request.user.is_authenticated:
            return render(
                request,
                "private_data/PRIVATE_DATA.html",
...
class PrivateDataDetailView(View):
    ...
    def get(self, request, private_pk):
        private_data = get_object_or_404(
            PrivateDataModel, user=request.user, pk=private_pk
        )
        msg_text = (
            f"USERNAME: {decrypt(private_data.username)}<br>"
            f"PASSWORD: {decrypt(private_data.password)}"
        )
...
class CreateNewPrivateDataView(View):
    ...
    def get(self, request):
        ...
        return render(
            request, "private_data/CREATE_NEW.html", {"forms": forms}
        )
    ...
    def post(self, request):
        ...
        if forms.is_valid():
            ...
            PrivateDataModel.objects.create(
                user=request.user,
                title=cd["title"],
                username=encrypt(cd["username"]),
                password=encrypt(cd["password"]),
            )
            messages.success(
                request, "username and password saved", "green-600"
            )
