...
class MessageConsumer(WebsocketConsumer):
    def connect(self):
        ...
        self.group = get_object_or_404(
            MessageGroupModel, blog__title=self.blog_title
        )
        async_to_sync(self.channel_layer.group_add)(
            re.sub("[^0-9a-zA-Z]+", "_", self.blog_title), self.channel_name
        )  # to replace all non-alphanumeric string with _
    ...
    def receive(self, text_data=None, bytes_data=None):
        ...
        msg = MessageModel.objects.create(
            user=self.user, group=self.group, text=message
        )
        event = {"type": "msg_handler", "msg_id": msg.id}
        async_to_sync(self.channel_layer.group_send)(
            re.sub("[^0-9a-zA-Z]+", "_", self.blog_title), event
        )
    ...
    def msg_handler(self, event):
        ...
        self.send(
            render_to_string("messages/htmx_messages_owner.html", context)
        )
    ...
    def online_users_handler(self, event):
        ...
        self.send(
            render_to_string("messages/htmx_online_users.html", context)
        )
