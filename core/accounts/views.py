...
class SignupView(View):
    ...
    def dispatch(self, request, *args, **kwargs):
        if request.user.is_authenticated:
            messages.error(
                request, "you are already authenticated", "amber-600"
            )
    ...
    def get(self, request):
        ...
        messages.success(
            request, "example for password: Aaa123$$", "cyan-600"
        )
    ...
    def post(self, request):
        ...
        if forms.is_valid():
            ...
            check_response = check_signup_data(
                cd["name"], cd["email"], cd["password_1"], cd["password_2"]
            )
            if not check_response["mode"]:
                messages.warning(
                    request, check_response["message"], "cyan-600"
                )
                return render(
                    request, "accounts/SIGNUP.html", {"forms": forms}
                )
            user = CostumeUser.objects.create_user(
                name=cd["name"], email=cd["email"], password=cd["password_1"]
            )
            login(request, user)
            messages.success(
                request,
                "user created, you have 30 minutes to verify your account",
                "cyan-600",
            )
            return redirect("home:main_page")
        messages.success(
            request, "example for password: Aaa123$$", "green-600"
        )
        return render(request, "accounts/SIGNUP.html", {"forms": forms})
    ...
    def post(self, request):
        ...
        if forms.is_valid():
            ...
            if user is not None:
                ...
                return redirect("home:main_page")
            messages.warning(
                request, "username or password is wrong", "cyan-600"
            )
            return render(request, "accounts/LOGIN.html", {"forms": forms})
        return render(request, "accounts/LOGIN.html", {"forms": forms})
...
class ChangePasswordView(View):
    ...
    def get(self, request):
        ...
        return render(
            request, "accounts/CHANGE-PASSWORD.html", {"forms": forms}
        )
    ...
    def post(self, request):
        ...
        if forms.is_valid():
            cd = forms.cleaned_data
            check_old_password = request.user.check_password(
                cd["old_password"]
            )
            if check_old_password:
                check_response = check_password_strength(
                    cd["new_password1"], cd["new_password2"]
                )
                if check_response["mode"]:
                    ...
                    messages.success(
                        request, "password-changed and logged in", "green-600"
                    )
                    return redirect("home:main_page")
                else:
                    messages.warning(
...
class SendForgetPasswordTokenView(View):
    ...
    def get(self, request):
        ...
        return render(
            request,
            "accounts/forget-password/SEND-FORGET-PASSWORD.html",
            {"forms": forms},
        )
...
class ConfirmForgetPasswordView(View):
    ...
    def get(self, request, token):
        ...
        try:
            ...
            return render(
                request,
                "accounts/forget-password/CONFIRM-FORGET-PASSWORD.html",
                {"forms": forms},
            )
    ...
    def post(self, request, token):
        ...
        if forms.is_valid():
            ...
            try:
                ...
                messages.warning(
                    request, "token has been expired", "green-600"
                )
                return render(
                    request,
                    "accounts/forget-password/CONFIRM-FORGET-PASSWORD.html",
            ...
            user.save()
            messages.success(
                request, "password changed successfully", "cyan-600"
            )
            return redirect("home:main_page")
        return render(
            request,
...
class SendActivateTokenView(View):
    def dispatch(self, request, *args, **kwargs):
        if not request.user.is_authenticated:
            messages.error(
                request,
                "please login for send you"
                "activation-email with authenticated user",
                "amber-600",
            )
            return redirect("home:main_page")
        elif request.user.is_verify:
            messages.error(
                request, "your account has been verified before", "amber-600"
            )
            return redirect("home:main_page")
        return super().dispatch(request, *args, **kwargs)
    ...
    def get(self, request):
        ...
        send_email_activateUser.delay(
            request.user.name, request.user.email, token
        )
...
class ActivateUserView(View):
    ...
    def get(self, request, token):
        ...
        if user.id != request.user.id:
            messages.error(
                request,
                "the user that receive email is not"
                "the same with user who authenticated",
                "amber-600",
            )
            return redirect("home:main_page")
        user.is_verify = True
