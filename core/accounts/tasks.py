...
def send_email_forgetPassword(user_name, user_email, token):
    ...
    for host in settings.CORS_ALLOWED_ORIGINS:
        ...
        email = EmailMessage('incs/email/forget_password.tpl', data, settings.EMAIL_HOST_USER, [user_email])
        email.send()
...
def send_email_activateUser(user_name, user_email, token):
    ...
    for host in settings.CORS_ALLOWED_ORIGINS:
        ...
        email = EmailMessage('incs/email/activate_user.tpl', data, settings.EMAIL_HOST_USER, [user_email])
        email.send()
...
def delete_unverified_users():
    ...
    CostumeUser.objects.filter(created__lt=expire_time, is_verify=False).delete()
